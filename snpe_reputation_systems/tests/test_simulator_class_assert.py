...
from hypothesis.strategies import integers, floats, none, text, tuples
...
class TestBaseSimulator:
    ...
    def get_base_simulator(review_prior =  np.array([1, 1, 1, 1, 1]),
    ...
    @given(arrays(int, 5, elements=integers(min_value=0, max_value=100)),
           arrays(arrays(dtype=int, shape=(integers(1, 10),), elements=integers(min_value=0, max_value=100)),
    ...
    def test___init__(array_int5, array_not5, random_string):
        ...
        assert isinstance(TestBaseSimulator.get_base_simulator(review_prior = array_int5), 
                          BaseSimulator)
        
        assert isinstance(TestBaseSimulator.get_base_simulator(simulation_type = "histogram"), 
                          BaseSimulator)
    ...
    @settings(max_examples=10)
    @given(arrays(dtype=int, shape=(integers(1, 10),), elements=integers(min_value=0, max_value=100)),
           arrays(dtype=int, shape=5, elements=integers(min_value=0, max_value=100)),
           arrays(int, 0))

    def test_convolve_prior_with_existing_reviews(array_not5, array_int5, empty_arr, none_value):
